                        ...
                        <td>
                            @if (Auth::user()->role <= 1)
                                <form action="{{ route('tasks.destroy', $task->id) }}" method="POST">
                                    ...
                                    <a class="btn btn-primary  my-1" href="{{ route('tasks.edit', $task->id) }}">Edit</a>

                                    @csrf
                                    @method('DELETE')

                                    <button type="submit" class="btn btn-danger  my-1" @if ($task->state == 0) disabled @endif
                                        onclick="if(!confirm('Are you sure to delete this task {{ $task->name }}?')) return false ">
                                        Delete
                                    </button>
                                @else
                                    <a class="btn btn-primary  my-1" href="{{ route('tasks.change', $task->id) }}">Change
                                        status</a>
